...
const searchFirst = (grid, firstChar) => {
    const matches = [];
    for (const row of grid.entries()) {
        const currMatches = findAll(row[1], firstChar);
        for (const hit of currMatches.entries())
            matches.push({ 'y': row[0], 'x': hit[1], 'direction': '', 'len': 1 });
    }
    return matches;
};
...
const searchSecond = (grid, secondChar, lastResults) => {
    const matches = [];
...
const searchCont = (grid, word, lastResults) => {
...
*/
const search = (grid, word, lastWord, lastResults) => {
    if (!word)
        throw 'No word provided.';
